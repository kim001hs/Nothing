...
def load_wdbc_data(filename):
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            wdbc.data.append(list(map(float, items[2:32])))  # TODO #1) Add 30 attributes (as floating-point numbers)
...
if __name__ == '__main__':
    ...
    # Train a model
    model = tree.DecisionTreeClassifier()                  # TODO #2) Find a better classifier (SVC accuracy: 0.902)
    ...
    conf_disp = metrics.ConfusionMatrixDisplay(confusion_matrix=conf_matx, display_labels=wdbc.target_names)
    conf_disp.plot()
